...
const CONTRACT_NAMES = ['MarketplaceFeeManager', 'NFTCollectionFactory', 'Marketplace'];
 ...
 */
function copyABIs() {
  const serverArtifactsPath = path.join(__dirname, '../../server/artifacts/contracts');
  ...
  for (const contractName of CONTRACT_NAMES) {
    try {
      ...
      const artifactPath = path.join(serverArtifactsPath, `${contractName}.sol`, `${contractName}.json`);
      ...
      console.error(`❌ Failed to copy artifact for ${contractName}:`, error.message);
    }
  }

  if (copiedCount === 0) {
    throw new Error('No artifact files were copied. Ensure contracts are compiled.');
  }

  console.log(`🎉 Successfully copied ${copiedCount}/${CONTRACT_NAMES.length} complete artifacts`);
}

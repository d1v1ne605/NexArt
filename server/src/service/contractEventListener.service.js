...
const nftCollectionFactoryABI = JSON.parse(
  readFileSync(
    resolve(
      __dirname,
      "../../artifacts/contracts/NFTCollectionFactory.sol/NFTCollectionFactory.json",
    ),
    "utf-8",
  ),
).abi;
...
class ContractEventListener {
  constructor() {
    ...
    this.NFT_COLLECTION_FACTORY_ADDRESS =
      process.env.NFT_COLLECTION_FACTORY_ADDRESS;
  ...
  async init() {
    try {
      ...
      if (this.MARKETPLACE_ADDRESS) {
        ...
        console.log(
          `📋 Marketplace contract initialized at: ${this.MARKETPLACE_ADDRESS}`,
        );
      } else {
        console.warn(
          "⚠️  MARKETPLACE_ADDRESS not found in environment variables",
        );
      }
      ...
      if (this.NFT_COLLECTION_FACTORY_ADDRESS) {
        ...
        console.log(
          `🏭 NFT Collection Factory initialized at: ${this.NFT_COLLECTION_FACTORY_ADDRESS}`,
        );
      } else {
        console.warn(
          "⚠️  NFT_COLLECTION_FACTORY_ADDRESS not found in environment variables",
        );
      }
    } catch (error) {
    ...
    try {
      ...
      for (const [key, listener] of this.listeners) {
        if (key.startsWith("TokenMinted_") || key.startsWith("Transfer_")) {
          ...
          const nftContract = this.nftCollectionContracts.get(
  ...
  async sendListingNotification(eventData, nftMetadata) {
    try {
      ...
      await notificationService.createNFTListingNotification(
        eventData,
        nftMetadata,
      );
  ...
  async queryHistoricalEvents(eventName, fromBlock = 0, toBlock = "latest") {
    try {
      console.log(
        `Querying historical ${eventName} events from block ${fromBlock} to ${toBlock}`,
      );
